...
**Field changes:** Replace `bgFar`/`bgNear` fields with `bgStars`/`bgTerrain`/`bgHaze` (all `Phaser.GameObjects.TileSprite`).
...
Places purely visual background structures (no physics) across the world. All at **depth 2.5** (behind player/enemies), scrollFactor 0 (parallax handled in `updateParallax` separately — these use the scene camera naturally at depth < player).

Actually: these use Phaser's default camera scroll (they're world objects, not fixed). Set them at depth 2 so they're behind platforms (depth 3–4) and players (depth 10+). They scroll with the camera naturally.
...
- Mast tilted: apply `setAngle(hash * 12 - 6)` (−6° to +6° lean)
...
Remove the `waveCleared` camera expansion block for L2 (the `cameraBoundMaxY += 480` logic).
...
### L2 Platforms
Reuse `makePlatforms()` with a new `'purple'` palette passed as argument. Update `makePlatforms()` to accept an optional palette parameter (default `'blue'`):
...
L2 platform counts: low band 18, mid band 15, high band 10 (+25% vs L1).
