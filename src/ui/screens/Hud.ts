...
class AbilityCard {
  ...
  constructor(scene: Phaser.Scene, readonly x: number, readonly y: number, name: IconName, key: string) {
    ...
    this.ring = new TickRing(scene, cx, cy, 36, 16, 4);
    this.ring.g.setDepth(DEPTH.hud + 1);
    this.glyph = icon(scene, cx, cy, name, 4, 'accent').setDepth(DEPTH.hud + 1);
    this.cap = new Keycap(scene, 0, 0, key, 'inkDim');
    this.cap.setPosition(x + 10 + this.cap.capW / 2, y + H - 20).setDepth(DEPTH.hud + 2);
    this.value = label(scene, x + W - 10, y + H - 20, '', 'small', 'ink').setOrigin(1, 0.5).setDepth(DEPTH.hud + 2);
  }

  static readonly W = 132;
  static readonly H = 124;
...
export class CombatHud {
  ...
  constructor(private readonly scene: Phaser.Scene, opts: HudOptions) {
    ...
    // ── ability dock ─────────────────────────────────────────────────────
    const gap = 12, n = 5, W = AbilityCard.W;
    const x0 = Math.round((GAME_W - (n * W + (n - 1) * gap)) / 2 / 2) * 2;
    const y0 = GAME_H - 28 - AbilityCard.H;
    ...
    for (const c of [this.turret, this.gatling, this.missile, this.repair, this.surge]) {
      keep(c.bg); keep(c.glyph); keep(c.ring.g); keep(c.cap); keep(c.value);
    }
  ...
  tick(time: number, dt: number, cam: Phaser.Cameras.Scene2D.Camera, playerX: number): void {
  ...
  setVisible(v: boolean): void { for (const o of this.all) o.setVisible(v); }
}
